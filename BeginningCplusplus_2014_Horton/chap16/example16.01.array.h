...
inline Array<T>::Array(const Array& array) try : size {array.size}, elements {new T[array.size]}
{
 ...
 catch (std::bad_alloc&) { std::cerr << "memory allocation failed for Array object copy.\n"; }
// Destructor
template <typename T> inline Array<T>::~Array() { delete[] elements; }
// Subscript operator
...
try
{
  ...
  {                                         // ...do the assignment...
    ...
    for (size_t i {}; i < size; ++i) elements[i] = rhs.elements[i];
  }
